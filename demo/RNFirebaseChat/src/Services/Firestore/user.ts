...
import {FireStoreCollection, IConversation, IUserProfile} from './type';

const createUserProfile = async (userId: string, name: string) => {
  const userRef = firestore()
    .collection<IUserProfile>(FireStoreCollection.users)
    .doc(userId);
      ...
      .then(snapshot => {
        return snapshot.docs.map(d => {
          if (d.data().memberRef) {
            d.data()
              .memberRef?.get()
              .then((r: QueryDocumentSnapshot<IUserProfile>) => {
                list.push({
                  ...d.data(),
